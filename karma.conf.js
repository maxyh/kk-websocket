...
var createWsServer = function(args, config, logger) {

    const log = logger.create('ws.server');
  
    log.info('Starting websocket server...');
  
    let port = typeof config.wsServer.port !== 'undefined' ? config.wsServer.port : 8889;

    const wss = new WebSocket.Server({ port: port }, () => {
        log.info(`websocket server started on port ${port}`);

        // Add possibility to do something after server has started
        if(config.wsServer.afterStart) {
            config.wsServer.afterStart(wss);
        }
    });

    if(config.wsServer.beforeStart) {
        config.wsServer.beforeStart(wss);
    }
};
...
module.exports = config => {
                    ...
                    .on('bundled', (err, content) => {
                        ...
                        if (bundleDirpath) {
                            ...
                            fs.writeFileSync(path.join(bundleDirpath,
                                `kk-websocket.${Date.now()}.js`), content);
                        }
                    });
            }
        },
    ...
    const DEBUG_TEST = env.DEBUG_TEST;
    if (DEBUG_TEST > 0) {
        cfg.browsers = [];
        cfg.browserify.transform = [];
    }
